...
class RecipeListFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        adapter = RecipeListAdapter(recipeList) { recipe ->
            ...
            Toast.makeText(requireContext(), "Clicked: ${recipe.cookMethode}", Toast.LENGTH_SHORT).show()
        }
    ...
    private fun fetchRecipesForFish(fishName: String) {
        lifecycleScope.launch {
            try {
                ...
                // Tangani error
                Toast.makeText(requireContext(), "Failed to load recipes: ${e.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
